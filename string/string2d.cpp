...
int main(int argc, char* argv[])
{
    ...
    // int N = 1000; // the number of nodes, must be odd currently
    int mode = 4; // the mode of the string (number of troughs + peaks)
    int stepSize = 100; // used for speeding up anim
    double mu = 4; // the linear mass density of the string
    double Lo = 1; // the length of the unstretched string
    double a = 10; // the stiffness 
    double T = 10; // initial tension along the string
    // double amp = 0.5; // the amplitude of the string
    double sigma = 0.4; // used for the Gaussian deformation
    double r = 1e-2; // the radius of each node




    double kstring = a/Lo; // spring constant of the whole string
    double k = N*kstring; // spring constant of segments of string
    double m = mu*Lo/N; // mass of each node
    double L = (T/kstring)+Lo; // length of stretched wire
    double ro = Lo/(N-1); // neutral distance between each node
    double v = sqrt((T*L)/(Lo*mu)); // analytic velocity of waves
    double w = 2*L/mode; // wavelength
    double pan = w/v; // analytic period
    ...
    // double maxU = 0;
        ...
        // printf("!%Lf\n",pan); // shows the appproximated frequency
        // printf("%Lf\n",pan);
        ...
        while(diff == 0) {
            ...
            for(i = 1; i < (N-1); i++) { // update the velocity of each node
                node[i].v += springForce(node[i-1].s,node[i].s,node[i+1].s,ro,k)*(dt/m);
            }
