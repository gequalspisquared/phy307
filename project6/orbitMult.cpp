...
#define NUMBODIES 10 // the number of bodies in our simulation
...
int main() 
{
	...
	Body body[NUMBODIES]; //sets up initial conditions
	body[0] = Body(vec3d(0,0,0),vec3d(0,0,0),1,0.25);
	body[1] = Body(vec3d(1,0,0),vec3d(0,2.5*M_PI,0),3.3e-5,0.05);
	for(i = 2; i < NUMBODIES; i++)
	{
		body[i] = Body(vec3d((drand48()-.5)*5,(drand48()-0.5)*5,(drand48()-0.5)/10),vec3d(drand48()*2*M_PI,drand48()*2*M_PI,drand48()/3),1e-6,0.05);
	}
